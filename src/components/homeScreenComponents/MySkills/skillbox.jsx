...
function SkillBox({ image, iconColor, icon, title, subtitle, description }) {
  const { setShow } = useContext(showContext);
  const { setShowDetails } = useContext(showDetailsContext);
  const handleOnclick = () => {
    setShow(true);
    const detail = { description: description, image: image };
    setShowDetails(detail);
  };
  return (
    <div className="border border-zinc-800 p-4 flex flex-col bg-gray-300/10 rounded-md">
